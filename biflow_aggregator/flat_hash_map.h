...
public:
    ...
    iterator get_delete_candidate(Key && key)
    {
        size_t index = hash_policy.index_for_hash(hash_object(key), num_slots_minus_one);
        int i = index;
        EntryPointer current_entry = entries + ptrdiff_t(index);
        for (EntryPointer it = current_entry; it != end(); ++it)
        {
            if (!it->is_empty()) {
                iterator result = { it };
                return result;
    ...
    template<typename Key>
    iterator get_first_record1(Key && key)
    {
        size_t index = hash_policy.index_for_hash(hash_object(key), num_slots_minus_one);
        EntryPointer current_entry = entries + ptrdiff_t(index);
    ...
    template<typename Key, typename... Args>
    std::pair<iterator, int> emplace_no_grow(value_type& removed, Key && key, Args &&... args)
    {
        size_t index = hash_policy.index_for_hash(hash_object(key), num_slots_minus_one);
        EntryPointer current_entry = entries + ptrdiff_t(index);
        int8_t distance_from_desired = 0;
        ...
        return emplace_new_key_no_grow(distance_from_desired, current_entry, removed, std::forward<Key>(key), std::forward<Args>(args)...);
    }

    template<typename Key, typename... Args>
    std::pair<iterator, int> emplace_no_grow(Key && key, Args &&... args)
    {
        size_t index = hash_policy.index_for_hash(hash_object(key), num_slots_minus_one);
        EntryPointer current_entry = entries + ptrdiff_t(index);
        int8_t distance_from_desired = 0;
        ...
        {
            if (compares_equal(key, current_entry->value))
                return { { current_entry }, DUPLICATED };
        }
        value_type to_insert;
        return emplace_new_key_no_grow(distance_from_desired, current_entry, to_insert, std::forward<Key>(key), std::forward<Args>(args)...);
    }

    template<typename Key, typename... Args>
    std::pair<iterator, int> insert_no_grow(value_type& to_insert, Key && key, Args &&... args)
    {
        return emplace_no_grow(to_insert, std::forward<Key>(key), std::forward<Args>(args)...);
    }

    template<typename Key, typename... Args>
    std::pair<iterator, bool> insert(Key && key, Args &&... args)
    {
...
private:
    ...
    convertible_to_iterator erase_no_destroy(const_iterator to_erase)
            {
                EntryPointer current = to_erase.current;
                current->destroy_value();
                --num_elements;
    ...
    template<typename Key, typename... Args>
    SKA_NOINLINE(std::pair<iterator, int>) emplace_new_key_no_grow(int8_t distance_from_desired, EntryPointer current_entry, value_type& removed, Key && key, Args &&... args)
    {
        using std::swap;

        if (distance_from_desired == max_lookups || num_elements + 1 > (num_slots_minus_one + 1) * static_cast<double>(_max_load_factor))
        {
            return { current_entry, FULL }; 
        }
        else if (current_entry->is_empty())
        {
            current_entry->emplace(distance_from_desired, std::forward<Key>(key), std::forward<Args>(args)...);
            ++num_elements;
            return { { current_entry }, INSERTED };
        }
        value_type to_insert(std::forward<Key>(key), std::forward<Args>(args)...);
        //removed = to_insert;
